...
- [ ] 下载进度实时反馈给用户
- [ ] 下载完成后文件显示在待上传区域
- [ ] 点击"上传"后文件正确处理(调用现有的文件处理流程)
- [ ] 错误处理完善(网络错误、文件不存在、权限错误等)
- [ ] 下载临时文件存储在合适的位置
- [ ] 下载失败的文件有重试机制
- [ ] 内存占用控制合理(流式下载)
...
### 6.1 代码文件
- [ ] `app/api/obs/download/route.js` - OBS文件下载API
- [ ] `lib/custom/obs/file-import.js` - 文件导入转换模块
- [ ] `components/custom/obs/OBSBrowserDialog.js` - 修改(添加下载逻辑)
- [ ] `components/text-split/FileUploader.js` - 修改(处理OBS文件)
