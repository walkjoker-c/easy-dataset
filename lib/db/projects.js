...
export async function createProject(projectData) {
  try {
    ...
    // ========== CUSTOM START ==========
    // ISS-006: 支持createdByUserId和externalId字段
    // 定制说明: 创建项目时如果提供了createdByUserId或externalId,存储到数据库
    // 修改日期: 2025-11-18
    const createData = {
      id: projectId,
    ...
    // 如果提供了createdByUserId,添加到数据
    if (projectData.createdByUserId) {
      createData.createdByUserId = projectData.createdByUserId;
    }

    // 如果提供了externalId,添加到数据
    if (projectData.externalId) {
      createData.externalId = projectData.externalId;
    }
    // ========== CUSTOM END ==========
