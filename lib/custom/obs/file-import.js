...
function getMimeType(fileName) {
  ...
  const mimeTypes = {
    ...
    jsonl: 'application/jsonl', // JSONL格式
    xml: 'application/xml',
    html: 'text/html',
    htm: 'text/html',
  };
 ...
 * @param {string} options.env - 环境(dev/test/prod)
 * @param {string} options.agentType - AgentType
 * @param {string} options.folder - 文件夹(pending/completed)
 * @returns {File} File对象
 */
export function createFileFromBase64(base64Content, fileName, options = {}) {
  ...
  // CUSTOM: REQ-003 - TASK-005 - 添加OBS元信息用于文件移动
  if (options.env) {
  ...
  if (options.agentType) {
    Object.defineProperty(file, '_obsAgentType', {
  ...
  if (options.folder) {
    Object.defineProperty(file, '_obsFolder', {
 ...
 * @param {Object} obsMetadata - OBS元信息 { env, agentType, folder }
 * @returns {Promise<Array>} File对象数组
 */
export async function downloadAndConvertFiles(files, onProgress, obsMetadata = {}) {
  const results = [];
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (data.success) {
        // CUSTOM: REQ-003 - TASK-005 - 转换为File对象并附加元信息
        const file = createFileFromBase64(data.content, data.fileName, {
          key: data.key,
          env: obsMetadata.env,
          agentType: obsMetadata.agentType,
          folder: obsMetadata.folder,
        });
