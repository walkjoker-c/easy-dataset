...
export async function handleApiError(response, options = {}) {
  ...
  if (response.status === 401) {
    ...
    // ISS-006: 保留原始错误消息,支持admin项目的登录提示
    // 定制说明: 如果不允许跳转到登录页(如项目详情页),显示原始错误消息
    //          原始错误消息可能包含admin项目提示,前端会据此显示登录按钮
    // 修改日期: 2025-11-18

    if (!allowLoginRedirect) {
      // 不允许跳转的场景: 保留原始错误消息(可能是admin项目专用消息)
      const msg = errorMsg || '无权访问此项目';  // 优先使用API返回的错误消息
      console.log('[ErrorHandler] 401 Unauthorized, login redirect disabled, msg:', msg);
